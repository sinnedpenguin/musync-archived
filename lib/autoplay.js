...
const autoplay = async (player, track) => {
  ...
  if (track.sourceName === "spotify") {
    ...
    if (currentTrackResult.exception) {
      console.error(`Error searching for the current track on YouTube: ${currentTrackResult.exception.message}`);
      return;
    }

    if (currentTrackResult.tracks.length === 0) {
      console.log("No matching track found on YouTube.");
      return;
    }
    ...
    if (nextTrack) {
      ...
      console.log("No next track available on YouTube.");
    }
  } else {
