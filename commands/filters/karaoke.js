...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const filterEmbed = new EmbedBuilder()
      .setColor(config.embedColor)
      .setDescription(`:white_check_mark: | \`Karaoke\` filter is now ${player.filters.karaoke ? '`enabled`' : '`disabled`'}! Use ${config.commands.nowplaying} to see all enabled filters.`)
      .setTimestamp();
