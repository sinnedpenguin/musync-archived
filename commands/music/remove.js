  ...
  execute(interaction) {
    ...
    if (!voiceChannel) {
      const voiceChannelEmbed = new EmbedBuilder()
        ...
        .setDescription(':x: | You need to be in a voice channel to remove a song from the queue!')
    ...
    if (!sameVoiceChannel || voiceChannel.id !== sameVoiceChannel.id) {
      const sameVoiceChannelEmbed = new EmbedBuilder()
        .setColor(config.embedColor)
        .setDescription(':x: | You must be in the same voice channel to remove a song from the queue!');
    ...
    if (songNumber > player.queue.length) {
      const invalidNumberEmbed = new EmbedBuilder()
        .setColor(config.embedColor)
        .setDescription(':x: | Invalid song number. Please enter a valid number in the queue.');
    ...
    const removeEmbed = new EmbedBuilder()
      .setColor(config.embedColor)
      .setDescription(`:wastebasket: | Removed ${songNumber}. [${removedSong.title}](${removedSong.uri}) from the queue.`)
      .setTimestamp();
