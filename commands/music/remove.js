  ...
  async execute(interaction) {
    ...
    if (!voiceChannel) {
      const voiceChannelEmbed = new EmbedBuilder()
        ...
        .setDescription(`:x: | You need to be in a voice channel to ${config.commands.remove} a song from the ${config.commands.queue}!`)
    ...
    if (!player || !player.queue || player.queue.length === 0) {
      const noSongsEmbed = new EmbedBuilder()
        .setColor(config.embedColor)
        .setDescription(`:x: | No songs are currently in the ${config.commands.queue}!`);
    ...
    if (!sameVoiceChannel || voiceChannel.id !== sameVoiceChannel.id) {
      const sameVoiceChannelEmbed = new EmbedBuilder()
        .setColor(config.embedColor)
        .setDescription(`:x: | You must be in the same voice channel to ${config.commands.remove} a song from the ${config.commands.queue}!`);
    ...
    if (songNumber > player.queue.length) {
      const invalidNumberEmbed = new EmbedBuilder()
        .setColor(config.embedColor)
        .setDescription(`:x: | Invalid song number. Please enter a valid number in the ${config.commands.queue}. Try again: ${config.commands.remove}.`);
    ...
    const removeEmbed = new EmbedBuilder()
      .setColor(config.embedColor)
      .setDescription(`:wastebasket: | Removed ${songNumber}. [${removedSong.title}](${removedSong.uri}). Use ${config.commands.queue} to see the current order.`)
      .setTimestamp();
