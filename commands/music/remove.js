  ...
  async execute(interaction) {
    ...
    if (!voiceChannel) {
      const voiceChannelEmbed = new EmbedBuilder()
        ...
        .setDescription(':x: | You need to be in a voice channel to </remove:1190439304183414882> a song from the </queue:1190439304183414881>!')
    ...
    if (!sameVoiceChannel || voiceChannel.id !== sameVoiceChannel.id) {
      const sameVoiceChannelEmbed = new EmbedBuilder()
        .setColor(config.embedColor)
        .setDescription(':x: | You must be in the same voice channel to </remove:1190439304183414882> a song from the </queue:1190439304183414881>!');
    ...
    if (songNumber > player.queue.length) {
      const invalidNumberEmbed = new EmbedBuilder()
        .setColor(config.embedColor)
        .setDescription(':x: | Invalid song number. Please enter a valid number in the </queue:1190439304183414881>. Try again: </remove:1190439304183414882>.');
