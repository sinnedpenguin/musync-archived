  ...
  async execute(interaction) {
    ...
    if (!voiceChannel) {
      const voiceChannelEmbed = new EmbedBuilder()
        ...
        .setDescription(`:x: | You need to be in a voice channel to ${config.commands.move} a song from the ${config.commands.queue}!`)
    ...
    if (!sameVoiceChannel || voiceChannel.id !== sameVoiceChannel.id) {
      const sameVoiceChannelEmbed = new EmbedBuilder()
        .setColor(config.embedColor)
        .setDescription(`:x: | You must be in the same voice channel to ${config.commands.move} a song from the ${config.commands.queue}!`);
    ...
    if (currentPosition > player.queue.length || newPosition > player.queue.length || currentPosition < 1 || newPosition < 1) {
      const invalidNumberEmbed = new EmbedBuilder()
        .setColor(config.embedColor)
        .setDescription(`:x: | Invalid song positions. Please enter valid positions in the ${config.commands.queue}. Try again: ${config.commands.move}.`);
    ...
    const moveMessage = messages.find(message =>
      ...
      message.embeds[0].description === `:arrow_double_up: | Moved [${movedSong.title}](${movedSong.uri}) from position ${currentPosition} to ${newPosition}. Use ${config.commands.queue} to see the current order.`
    );
    ...
    const moveEmbed = new EmbedBuilder()
      .setColor(config.embedColor)
      .setDescription(`:arrow_double_up: | Moved [${movedSong.title}](${movedSong.uri}) from position ${currentPosition} to ${newPosition}. Use ${config.commands.queue} to see the current order.`)
      .setTimestamp();
